	...
	namespace Secret
	{
		...
		VarDecl* VarDecl::TryCreate(TokenContainer* pTokenContainer, OffsetDatum* pOffsetDatum, bool bInline)
		{
			...
			if (pSquareBracketLeftToken != 0)
			{
				...
				{
					NumberDuck::Secret::AST* __2516344687 = pArraySize;
					pArraySize = 0;
		...
		void VarDecl::Stringify(Language eLanguage, OutputFile eOutputFile, int nDepth, InternalString* sOut)
		{
			...
			if (m_pAssignment != 0)
			{
				...
				if (m_pParent != 0 && m_pParent->m_eType == AST::Type::AST_PARAM_DECL && eLanguage == AST::Language::CPP && eOutputFile == AST::OutputFile::SOURCE)
					bDoIt = false;
				if (bDoIt)
				{
					sOut->AppendString(" = ");
